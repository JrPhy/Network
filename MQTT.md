相較於 HTTP，MQTT 是一種發布(publish)-訂閱(subscribe)的模式，並由一個 BROKER 來管理誰訂閱了什麼主題，把這些資料保存在本地，在傳遞封包時的大小就會小非常多，約只有 HTTP 的 1/10 到 1/20，畢竟 MQTT 已經知道對方是誰了，所以在效能上會比 HTTP 還要好。實務上也不希望中間連線有中斷，所以會一直保持連線。
